...
exports.login = asyncHandler(async (req, res, next) => {
    ...
    const isMatch = await user.matchThePasswords(password)
    if (!isMatch) {
        return next(new ErrorResponse('Invalid credential', 401))
    }
...
// @access Public
exports.updateUserDetails = asyncHandler(async (req, res, nex) => {

    const FildesToUpdate ={
        name:  req.body.name,
        email: req.body.email
    }


    const user = await User.findByIdAndUpdate(req.user.id,FildesToUpdate,{
        new: true,
        runValidators: true
    })
    res.status(200).json({
...
exports.updatePassword = asyncHandler(async (req, res, nex) => { 
    ...
    //Cheack current password
    if(!(await user.matchThePasswords(req.body.currentPassword))){
